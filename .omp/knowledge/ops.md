...
构建期 dart-define：`APP_VERSION`、`ANALYTICS_APP_KEY`、`STATS_*`（`FLUXCLOUD_BASE_URL` 已随云层移除废弃；web 侧残留 `VITE_FLUXCLOUD_BASE_URL` 待清理）。
...
- **已移除（cf05dd7）**：多设备协作 / FluxCloud 配置同步（`multi-device-collab-design.md`）——Flutter 侧 `lib/src/services/cloud/` 及配套对话框/测试已整体删除，Rust `native/agent` 明确为纯本地边界；`web/src/lib/cloud/` 为残留待清理。**本地 headless server 无任何 cloud/sync 路由**。
