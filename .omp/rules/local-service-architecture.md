...
- `native/engine`：下载领域实现；不知道 UI 和进程传输。
- `native/daemon`（`fluxdown_daemon`，目标二进制 `fluxdownd`）：aria2c 式纯下载核心；拥有下载任务、下载设置、RSS、插件、Webhook、下载 DB 与下载事件。
- `native/agent`（`fluxdown_agent`，目标二进制 `fluxdown-agent`）：官方客户端常驻本地代理；仅承载本地 UI Gateway。**纯本地：不含账户、云同步、设备协同或任何外部云依赖**（云层已整体移除）。
...
- daemon/agent 均为纯本地边界：任何账户凭据、云同步 revision、设备信任或远程任务状态机都不得进入这两个 crate；出现此类需求先停下来确认契约变更。
...
- 云功能已从 Flutter `lib/src/services/cloud/` 整体移除（不迁入 agent）；不得在 agent、daemon 或任何客户端重建云同步/账户状态机。
- 只为真实迁移切片创建模块；禁止预建空目录、空 trait、占位 handler、假成功响应或无运行行为的二进制。

## 修改前检查

1. UI 全退后仍需继续吗？需要则归 daemon 或 agent，不归 UI。
2. 功能是否引入外网账户或云服务依赖？是则停止——daemon/agent 均为纯本地边界，新增云能力需要显式契约变更。
